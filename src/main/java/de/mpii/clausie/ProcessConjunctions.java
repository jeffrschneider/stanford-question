...
public class ProcessConjunctions {
	...
	private static final boolean EXTRA = false;
    ...
    private static void generateConstituents(SemanticGraph semanticGraph, Tree depTree,
		...
		if (conjunct != null) {
			...
			// added by grv - check equals the other way round - no nulls.
			...
			// commented out by grv - check that get is not null as well
        ...
        for (SemanticGraphEdge edge : outedges) {
            ...
            if (DpUtils.isAnyConj(edge) && processCC) {           	
				...
				// commented out by grv, check with no way for null pointer
                ...
                for (IndexedWord parent : parents) {
					...
					// Added is extra to comply with newer version of stanford parser
					// set it to true. - aka the last parameter
                    newSemanticGraph.addEdge(parent, newRoot, reln, weight, EXTRA);
                }
                ...
                for (SemanticGraphEdge ed : outedges) {
                    ...
                    if(DpUtils.isPredet(ed) && "both".equals(ed.getDependent().lemma())) { //if it is one level down
						...
						// Added is extra to comply with newer version of stanford parser
						// set it to true. - aka the last parameter
                        newSemanticGraph.addEdge(newRoot, child, reln, weight, EXTRA);
                    }
                }
				...
				// commented out by grv - perform check for equals that doesnt throw null
            // } else if ((DpUtils.isCc(edge) || DpUtils.isPreconj(edge))&& processCC && !edge.getDependent().lemma().equals("&")) {
            } else if ((DpUtils.isCc(edge) || DpUtils.isPreconj(edge))&& processCC && !"&".equals(edge.getDependent().lemma())) {
                semanticGraph.removeEdge(edge);
    ...
    public static List<IndexedWord> getIndexedWordsConj(SemanticGraph semanticGraph, Tree depTree,
        ...
        for (SemanticGraphEdge edge : outedges) {
            if (edge.getRelation().toString().equals(rel.toString())) {
    	...
    	for(int i = 0; i < ccs.size(); i++) {
    		for(int j = i + 1; j < ccs.size(); j++) {
    	        ...
    	        for (SemanticGraphEdge ed : semanticGraph.getIncomingEdgesSorted(ccs.get(i))) {
					...
					// Added is extra to comply with newer version of stanford parser
					// set it to true. - aka the last parameter
    	            semanticGraph.addEdge(ed.getGovernor(), ccs.get(j), ed.getRelation(), ed.getWeight(), EXTRA);
    	        }
    	       ...
    	       // and assign them in each case.
    	        for (SemanticGraphEdge ed : semanticGraph.getOutEdgesSorted(ccs.get(i))) {
    	            ...
    	            if (!DpUtils.isAnyConj(ed) && !DpUtils.isCc(ed)
						...
						// Added is extra to comply with newer version of stanford parser
						// set it to true. - aka the last parameter
    	                semanticGraph.addEdge(ccs.get(j), child, ed.getRelation(), ed.getWeight(), EXTRA);
    	            }
    	        }
