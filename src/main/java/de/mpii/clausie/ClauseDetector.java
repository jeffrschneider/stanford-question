...
/**
 * {@link ClauseDetector} contains the methods dealing with the detection of
 * clauses. After the detection is performed a set of {@link Clause} is created.
 *
 * {@code detectClauses} first detects the type of clause to be generated based
 * on syntactic relations and once a clause is detected a given method is used
 * to create a {@link Clause}.
 *
...
class ClauseDetector {
	...
	protected static final boolean EXTRA = false;
    ...
    static boolean detectClauses(ClausIE clausIE) {
        ...
        for (SemanticGraphEdge edge : clausIE.semanticGraph.edgeIterable()) {
            // Added by Greg to detect conditional clauses
            if (clausIE.semanticGraph.isInConditionalContext(edge.getGovernor())) {
                System.out.println("~~~~~STOP ONE~~~~~");
                return false;
            } else {
                // check whether the edge identifies a clause
                if (DpUtils.isAnySubj(edge)) {
                    System.out.println("~~~~~ANY SUBJ ONE~~~~~");
                    ...
                    System.out.println("~~~~~ANY APPOS ONE~~~~~");
                    ...
                    System.out.println("~~~~~ANY POSS ONE~~~~~");
                    ...
                    System.out.println("~~~~~ANY PART ONE~~~~~");
        ...
        for (int i = 0; i < clausIE.clauses.size(); i++) {
			...
			// added by grv - the check was inside else - but didnt check
			// gregs added code - which was a potential null pointer exception
			if (root != null) {
				// Added by Greg to detect wish clauses
				String stativeVerb = clause.constituents.get(clause.verb).rootString();
//                System.out.println("~~~~~STATIV VERB" + stativeVerb + "~~~~~");
				if (clausIE.options.dictStativeVerbs.contains(stativeVerb)) {
                    System.out.println("~~~~~STOP TWO~~~~~");
					return false;
				} else if (root.originalText().equals(stativeVerb) && clausIE.options.dictStativeVerbs.contains(root.lemma())) {
                    System.out.println("~~~~~STOP THREE~~~~~");
				    return false;
				} else {
					// set parents (slow and inefficient for now)
					// moved this if outside gregs code - as this check is needed -grv
					//if (root != null) {
					...
					//closes if (root != null) - commented away by grv
					//}
        ...
        System.out.println("~~~~~STOP FOUR~~~~~");
        return true;
    }

    /**
     * Adds in the exclude vertex of a clause the head of the rest of the
     * clauses
     */
    ...
    /**
     * Selects constituents of a clause for clauses with internal subject or
     * coming from a participial modifier
     *
     ...
     * @param partmod Indicates if the clause is generated from a partmod
     * relation
     */
    private static void addNsubjClause(ClausIE clausIE, List<IndexedWord> roots,
        ...
        for (int i = 0; i < ccs.size(); i++) {
            ...
            // relative clause?
            System.out.println("INCOMING EDGES " + incomingEdges); // TODO: the problem
...
//------------------------Set verb or complement, and subject.-------------------------------------------------
            ...
            System.out.println("SUBJECT " + subject.word() + " " + subject.tag().charAt(0) + " " + rcmod);
            if (subject.tag().charAt(0) == 'W' && rcmod != null) {
                ...
                rcmod = null;
                System.out.println("%%%%%%%SUBJECT IF ONE");
                ...
                clause.constituents.add(createPossConstituent(semanticGraph, poss, rcmod, subject,
                        Type.SUBJECT));
                rcmod = null;
                System.out.println("%%%%%%%SUBJECT IF TWO");
            ...
            if (partmod) {
                ...
                if (coppm != null) {
                    ...
                    SemanticGraphEdge spm = DpUtils.findFirstOfRelationOrDescendent(outsub,
                            EnglishGrammaticalRelations.SUBJECT);
					// added by grv - added if - spm can be null - look at above method..
					if (spm != null) {
						((IndexedConstituent) clause.constituents.get(clause.subject)).excludedVertexes
            ...
            for (SemanticGraphEdge outgoingEdge : outgoingEdges) {
                ...
                if (DpUtils.isMark(outgoingEdge)) {
					...
					// change for corenlp 3
						)) {
                    System.out.println("%%%%%%%%%%%%%%%%%VARIOUS ADVERBALS");
            ...
            if (rcmod != null) { //"I saw the house I grew up in", "I saw
                ...
                if (candidate != null) {
					...
					// added true to end to update to corenlp 3
...
//------------------------------------------------------------------------------------------------------------------          
    ...
    private static Constituent createPossConstituent(SemanticGraph semanticGraph,
        ...
        newSemanticGraph.addEdge(poss.getGovernor(), rcmod.getGovernor(),
                EnglishGrammaticalRelations.POSSESSION_MODIFIER, weight, EXTRA);
     ...
     *
     * @param subject The subject of the clause (first argument of the appos
     * relation)
     * @param object The object of the clause (second argument of the appos
     * relation)
     */
