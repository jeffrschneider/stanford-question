...
public class Options {
	...
	public Dictionary dictAdverbsInclude;
        public Dictionary dictStativeVerbs;
...
//	public boolean processPassive = false; // NOT SUPPORTED FOR NOW (collapsed semantic graph needed but less stable)
	...
	public Options() {
		try {
			InputStream in = classLoader.getResource("clausie.conf").openStream();
			setOptions(in);
			in.close();		
		} catch (IOException e) {
			// should not happen
	...
	public void setOptions(InputStream optionsStream) throws IOException {
		...
		dictAdverbsInclude = getDictionary(prop, "dictAdverbsInclude");
                dictStativeVerbs = getDictionary(prop, "dictStativeVerbs"); // Added by Greg
	...
	public void print(OutputStream out, String prefix) {
		...
		pout.println(prefix + "  Dict. stative verbs   : " + dictStativeVerbs.size() + someWords(dictStativeVerbs.words));
